...
Everything here is a **frozen snapshot**. Nothing self-updates — MedBud's index and sitemap are
Cloudflare-blocked, so the data is captured by hand and shipped with the extension. It works well and
decays quietly as the two sites change, mostly by reverting to the search fallback. This is the guide to
keeping it fresh.

## The 30-second model

A card's link is resolved in this order, first hit wins:
...
3. **Search fallback** — a search scoped to `medbud.wiki` (or `leafly.com/strains` for the Leafly link).
   Always works, never breaks; this is what stops anything from ever fully failing.

The **cache** just remembers the answer per product name, keyed with `RESOLUTION_VERSION`. It matches
nothing.
...
| **All** cards link to search after an update | Stale cache shadowing new logic | Bump `RESOLUTION_VERSION` (Recipe E) |
| **No badges at all** | CB1 reskinned the portal | Recipe D |
| Everything shows *MedBud check needed* | Live ratings on + Cloudflare | Turn live ratings off in options (it is off by default) |

## Recipes

All recipes start by saving the relevant page — **Save page as… → Webpage, Complete** — while signed in
...
Regenerates `src/data/medbud-index.json`. It **refuses to shrink** the formulary, so a half-loaded save
...
verified against MedBud's own index pages, not trusted from a search snippet. This is the method that
built the current mapping:
...
2. List the CB1 products that still fall back (open the service-worker console, or diff against the
   mapping keys).
3. For each, find its path by grepping the saved MedBud index for the brand, and confirm the slug names
   the same strain:
   ```bash
...
5. Confirm every path you added is real:
   ```bash
   # each mapped path should appear in a saved MedBud index page
   grep -F "/oils/curaleaf/peppermint-t20c40/" "path/to/All Sublingual Oils … .html"
   ```
...
- **If it is a matcher match** (flower, not in the mapping): it is a scoring bug. Add the product name
  ...
  "does not cross brands…" tests), then tighten `product-matcher.js` until it rejects. Then Recipe E.
...
Every user's cached results from the old logic are then ignored on next load — no manual cache clear.
Forgetting this is why "I fixed it but it still shows the old link" happens.

## When you need code, not data
...
**Verify before you ship.** Every path that reaches a card should have been grepped out of a real saved
MedBud page. The search fallback is always there to catch a miss — but a *wrong* direct link has no
safety net, and this points at medicine.
