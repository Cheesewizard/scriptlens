...
**GreenLens does not collect, store, or transmit any personal or medical data.** There is no analytics,
no tracking, no account, and no server that belongs to GreenLens.

This is a short, plain-English policy for a tool that runs entirely in your browser.

## What it reads

On the CB1 Medical portal (`patient.cb1medical.com`), GreenLens reads the **product name** shown on each
...
- **Nothing goes to a GreenLens server, because there isn't one.**
- **Search fallback:** when a product is not in the bundled data, GreenLens builds a web-search link from
...
- **Optional direct resolution:** if you add your own Brave Search API key in the settings, GreenLens
...
No request GreenLens makes carries any information about you — only the public name of a medication.
